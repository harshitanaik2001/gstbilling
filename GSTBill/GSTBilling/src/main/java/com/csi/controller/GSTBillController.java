...
public class GSTBillController {
    ...
    @GetMapping("/searchbyinvoicedate/{invoiceDate}")
    @Operation(summary = "Search Bill Using Invoice Date", description = "API for search bill using invoice date. " +
            "Input should be  Invoice Date",
            responses = @ApiResponse(responseCode = "401", content = @Content(schema = @Schema(example = "Unauthorized. Please login and provide token in header"))))
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<List<GSTBill>> searchByInvoiceDate(@PathVariable String invoiceDate) {
        return ResponseEntity.ok(gstBillService.searchByInvoiceDate(invoiceDate));
    }
}